...
of pre-provisioned appliance VMs. The size of this pool will be configurable
...
currently searches for a router's corresponding Nova VM by name (ie,
...
We will need to add a layer of abstraction around the current VM Manager
``akanda.rug.vm_manager.VmManager`` and its integration points with the
state machine. Instead of requesting node resources directly from the
...
VmManager and exposes the main methods used by the state machine, namely:
